...
export default function FindingsPanel({ findings, model, selectedId, onSelect }: {
  findings: Finding[]; model: ArchitectureModel; selectedId: string | null; onSelect: (f: Finding) => void
}) {
          ...
          {rows.map(f => (
            <li key={f.id} onClick={() => onSelect(f)}
              ...
              <div className="flex items-center gap-2">
                ...
                {f.source === 'Ai' && <span className="ml-auto flex items-center gap-1 text-[10px] text-accent"><Sparkles size={11} /> AI</span>}
                {f.source === 'Rule' && <span className="ml-auto font-mono text-[10px] text-muted">{f.ruleId}</span>}
              </div>
